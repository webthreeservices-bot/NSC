"use strict";(()=>{var a={};a.id=503,a.ids=[503],a.modules={261:a=>{a.exports=require("next/dist/shared/lib/router/utils/app-paths")},3295:a=>{a.exports=require("next/dist/server/app-render/after-task-async-storage.external.js")},5486:a=>{a.exports=require("bcrypt")},10846:a=>{a.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},18490:(a,b,c)=>{c.a(a,async(a,d)=>{try{c.r(b),c.d(b,{GET:()=>i});var e=c(45592),f=c(8216),g=c(75775),h=a([f,g]);async function i(a){let b=await (0,g.Xp)(a);if(b instanceof e.NextResponse)return b;let{user:c}=b,d=(0,g.ZT)(c);if(d instanceof e.NextResponse)return d;try{let[a,b,c,d,g,h,i,j,k]=await Promise.all([(0,f.P)('SELECT COUNT(*)::int as count FROM "User"'),(0,f.P)(`
        SELECT COUNT(*)::int as count
        FROM "User"
        WHERE "lastLogin" >= $1
      `,[new Date(Date.now()-2592e6)]),(0,f.P)('SELECT COUNT(*)::int as count FROM "UserPackage"'),(0,f.P)(`
        SELECT COUNT(*)::int as count
        FROM "UserPackage"
        WHERE status = 'ACTIVE'
      `),(0,f.P)(`
      ...
      `)]),l=a[0]?.count||0,m=b[0]?.count||0,n=c[0]?.count||0,o=d[0]?.count||0,p={_sum:{amount:g[0]?.total||"0"}},q={_sum:{netAmount:h[0]?.total||"0"}},r=i[0]?.count||0,s={_sum:{amount:j[0]?.total||"0"}},t=k[0]?.count||0,u=await (0,f.P)(`
    ...
    `),v=(await (0,f.P)(`
      ...
      LIMIT 5
    `)).map(a=>({id:a.id,amount:a.amount,packageType:a.packageType,status:a.status,createdAt:a.createdAt,user:{email:a.userEmail,username:a.username}})),w=await (0,f.P)(`
      SELECT
        "packageType",
        COUNT(*)::int as count,
